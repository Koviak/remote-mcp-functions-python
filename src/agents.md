...
- Teams chat read surface: `endpoints/teams.py` serves `GET me/chats`, `GET chats/{chat_id}`, `GET chats/{chat_id}/members`, and `GET chats/{chat_id}/messages`; all five chat routes (including the `POST me/chats/messages` send/reply route) are listed in the route-table comment at `http_endpoints.py:2143-2147` and described in `Current_Implementation.md`. A route advertised by Annika's `office_teams_chat` ENDPOINT_MAPPING but not registered here returns a 404 that looks like missing data; `src/Tests/test_teams_contract_endpoints.py::test_chat_read_routes_are_registered_in_the_functions_app` is the detector for that class and must stay green.
